...
struct instructions {
	...
	int L;	/* L_code */
	int M;	/* M_code */
	int Line; /*for parser.c line number */
	int AR;     // store what AR is this for
	int AR_Index; // store the position for this index
	int procNumberId;   // for calls, what is the procedure id
	int Called;         // used to know if a procedure is called initially or as subsequent calls
	int jOffset;      // used to know how many lines to jump
};
...
// string representation of the symbols, char array starts at index 0, compensate with offset of 1
// with lexeme values
char *m_sa_token_Strings[] = {
    "nulsym", "identsym", "numbersym", "+", "-", "*",
    "/", "oddsym", "=", "!=", "<", "<=",
    ">", ">=", "(", ")", ",", ";",
    ".", ":=", "begin", "end", "if", "then",
    "while", "do", "call", "const", "var", "procedure",
    "write", "read", "else" };
...
typedef struct {
	...
	int level;                      // L level
	int adr;                        // M address
	int Line;                       // vm line number
	int procNumber;                 // what is the number of this procedure
