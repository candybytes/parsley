...
void proc_decl(){
    ...
    while (m_nCurrentToken == procsym) {
        ...
        getNextTokenNode();
        enterCode(jmp, 0, 0); // jump to line of parent
...
void process_STATEMENT(){
    ...
    switch (m_nCurrentToken) {
        ...
        case whilesym:
            ...
            // store the value of M after statement
            codeLines[m_nAR_Level][endWhileLine].M = m_nCodeLineCount;
...
void initializeInstructions(){
    ...
    for (j = 0; j < MAX_AR_LEVELS; j++) {
        //printf("I am inside LEVELS %d ", j);
        for (i = 0; i < MAX_CODE_LENGTH ; i++) {
            //printf("   I am inside LENGTH %d\n", i);
            codeLines[j][i].OP = -1;
            codeLines[j][i].L = 0;
            codeLines[j][i].M = 0;
            //printf("i %d, j %d, OP %d, L %d, M %d\n",i, j, codeLines[j][i].OP, codeLines[j][i].L, codeLines[j][i].M );
        }
        //printf("\n");
